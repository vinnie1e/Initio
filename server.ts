...
app.post("/api/gemini/generate-story", async (req, res) => {
  ...
  const resolveStoryGenre = (genreStr: string) => {
    if (!genreStr) return "Fantasy";
    const low = genreStr.toLowerCase();
  ...
  const getFallback = () => {
    const resolvedStyle = resolveStoryGenre(gStyle);
    ...
    const titlesByGenre: Record<string, string[]> = {
      ...
      "Pop-Culture": [
        "The Hero’s Journey",
        "Multiverse Crisis Sync",
        "Epic Quest of Champions",
        "The Legendary Artifact Hunt",
    ...
    const beatsByGenre: Record<string, Array<(t: string) => string>> = {
      ...
      "Pop-Culture": [
        (t: string) =>
          `Equip your gear for “${t}” — time to start the main quest.`,
        (t: string) =>
          `Defeating the mini-boss of “${t}” to unlock the next region of the map.`,
        (t: string) =>
          `Acquiring the rare loot from “${t}” — your stats are rapidly increasing.`,
        (t: string) =>
          `Solving the ancient puzzle of “${t}” to reveal the secret path.`,
        (t: string) =>
          `Forming a guild party to tackle “${t}” before the timer runs out.`,
        (t: string) =>
          `Unlocking the legendary achievement: completed “${t}” with flying colors!`,
      ],
    };

    const genreTitles = titlesByGenre[resolvedStyle] || titlesByGenre["Fantasy"];
    const genreBeats = beatsByGenre[resolvedStyle] || beatsByGenre["Fantasy"];
    ...
    const introText = introsByGenre[resolvedStyle] || introsByGenre["Fantasy"];
...
Special Pop Culture Integration Rules:
If the user's story prompt/goal or tasks contain pop culture references, franchise lore (e.g. Star Wars, Harry Potter, Middle-earth/Lord of the Rings, Marvel, DC/Batman, Zelda, Pokemon, anime/manga, Dune, Matrix, gaming/movies), or famous fictional tropes, enthusiastically embrace and integrate them! You are encouraged to mention specific characters, magic spells, weapons, or locations from those pop culture universes, creating a deeply immersive, custom, and delightful experience.
