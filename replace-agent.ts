...
const searchStr = 'app.post("/api/calendar/connect"';
const agentEnd = content.indexOf(searchStr);

const originalAgent = content.substring(agentStart, agentEnd);

// I will now replace `originalAgent` with the new implementation.

const newAgent = `export const agentFlow = aiGenkit.defineFlow({
  name: "agentFlow",
  ...
  const researchFactTool = aiGenkit.dynamicTool({
    ...
    const { query } = args;
    try {
      const searchRes = await aiGenkit.generate({
        model: "googleai/gemini-2.5-flash",
        prompt: query,
        tools: []
      });
      // Genkit google search plugin not installed, so using plain text fallback.
      const text = searchRes.text;
      const observeText = text || "No information found.";
      return { fact: observeText, _observeText: observeText };
    } catch(e: any) {
      return { error: String(e), _observeText: \`Search failed: \${String(e)}\` };
    }
  });
...
fs.writeFileSync('server.ts', content.substring(0, agentStart) + newAgent + '\n\n' + content.substring(agentEnd));
