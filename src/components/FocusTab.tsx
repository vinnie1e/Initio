...
export default function FocusTab({ db, patch, flash, isMobile, activeTasks, onComplete, selectedTaskId: propsSelectedTaskId, setSelectedTaskId: propsSetSelectedTaskId }: FocusTabProps) {
  ...
  const getGridData = () => {
    ...
    const monthsData: { monthName: string; days: { dateStr: string; minutes: number; count: number; dayOfMonth: number; isFuture: boolean }[] }[] = [];

    for (let mOffset = 2; mOffset >= 0; mOffset--) {
      const year = today.getFullYear();
      const month = today.getMonth() - mOffset;
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        const current = new Date(mYear, mMonth, d);
        const dateStr = current.toDateString();
        const isFuture = current > today;

        const daySessions = focusLog.filter(item => new Date(item.at).toDateString() === dateStr);
        ...
        <div className="card" style={{ padding: "20px 22px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 24px", background: "var(--surface-2)", borderRadius: "var(--radius)", border: "1px solid var(--line)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              {isRunning ? (
                <div style={{ fontSize: 52, fontWeight: 700, fontFamily: "var(--font-mono)", color: timeLeft === 0 ? "var(--good)" : "var(--text)", letterSpacing: -2, marginBottom: 8, lineHeight: 1 }}>
                  {formatTime(timeLeft)}
                </div>
              ) : (
                  ...
                  <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const hrs = isNaN(h) ? 0 : Math.max(0, h);
                        const currentMins = Math.floor((timeLeft % 3600) / 60);
                        const val = hrs * 3600 + currentMins * 60;
                        setTimeLeft(val);
                        setDuration(val);
                      }}
                      className="focusable"
                      style={{
                        fontSize: 52, fontWeight: 700, fontFamily: "var(--font-mono)", color: "var(--text)", letterSpacing: -2, lineHeight: 1,
                        background: "transparent", border: "none", width: 60, textAlign: "right", outline: "none", padding: 0
                      }}
                    />
                    <span style={{ fontSize: 24, fontWeight: 600, color: "var(--muted-2)", fontFamily: "var(--font-mono)" }}>h</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 2 }}>
                    <input
                      type="number"
                      ...
                      onChange={(e) => {
                        ...
                        const currentHrs = Math.floor(timeLeft / 3600);
                        const val = currentHrs * 3600 + mins * 60;
                        setTimeLeft(val);
                        setDuration(val);
                      }}
                      className="focusable"
                    ...
                    />
                    <span style={{ fontSize: 24, fontWeight: 600, color: "var(--muted-2)", fontFamily: "var(--font-mono)" }}>m</span>
                  </div>
                </div>
              )}
              ...
              <div style={{ minHeight: 24, marginBottom: 20, textAlign: "center", maxWidth: "90%" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ display: "flex", color: "var(--muted-2)" }}><MIcon name="volume_up" s={14} /></span>
                  <span className="mono" style={{ fontSize: 10, color: "var(--muted-2)", textTransform: "uppercase" }}>noise:</span>
                  <select
                    value={ambientSound}
        ...
        <div className="card" style={{ padding: "20px 22px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              ...
              <span style={{ width: 9, height: 9, borderRadius: 2, background: "rgba(255,255,255,0.04)" }} />
              <span style={{ width: 9, height: 9, borderRadius: 2, background: "var(--bad)" }} />
              <span style={{ width: 9, height: 9, borderRadius: 2, background: "var(--muted-2)" }} />
              <span style={{ width: 9, height: 9, borderRadius: 2, background: "var(--good)" }} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12, minWidth: 320 }}>
              {grid.map((month, mIdx) => (
                <div key={mIdx} style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <div className="mono text-[10px] text-var(--muted-2) uppercase">{month.monthName}</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                    {month.days.map((day, dIdx) => (
                      <div
                        key={dIdx}
                        onMouseEnter={() => !day.isFuture && setHoveredCell({ dateStr: day.dateStr, minutes: day.minutes, count: day.count })}
                        onMouseLeave={() => setHoveredCell(null)}
                        onClick={() => !day.isFuture && setHoveredCell({ dateStr: day.dateStr, minutes: day.minutes, count: day.count })}
                        className={day.isFuture ? "" : "tap"}
                        style={{
                          width: 14,
                          height: 14,
                          borderRadius: 2,
                          background: day.isFuture ? "transparent" : getCellColor(day.minutes),
                          border: day.isFuture ? "1px dashed var(--line)" : "none",
                          cursor: day.isFuture ? "default" : "pointer",
                          transform: "scale(1)",
                          boxShadow: "none"
                        }}
                        title={day.isFuture ? "" : `${day.dateStr}: ${day.minutes}m focused (${day.count} session${day.count !== 1 ? "s" : ""})`}
                      />
                    ))}
                  </div>
                </div>
