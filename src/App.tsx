...
function localStory(goal: string, tasks: string[], genre: string = "Fantasy") {
  ...
  if (goal) {
    ...
    if (/\b(space|star|cyber|robot|ai|hack|system|code|laser|subnet|terminal|digital|data|tech|synth|ship|spacecraft|grid|quantum|mainframe)\b/.test(low)) {
      gStyle = "Sci-Fi";
  ...
  return {
    ...
    intro: introsByGenre[gStyle] + ` Move through them, and the story bends toward you.`,
...
const GCal = {
  ...
  _tc: null as any,
  scope: "https://www.googleapis.com/auth/calendar.events https://www.googleapis.com/auth/calendar.readonly https://www.googleapis.com/auth/gmail.compose https://www.googleapis.com/auth/documents https://www.googleapis.com/auth/drive.file",
...
function SystemStatusBadge({ db, gcalConnected }: { db: AppState | null, gcalConnected: boolean }) {
...
function OnboardingTour({ onComplete, isMobile }: { onComplete: (seedDemo: boolean) => void; isMobile: boolean }) {
  ...
  const steps = [
    {
      id: "story",
      eyebrow: "01 / 05 • Interactive Story Mode",
    ...
    {
      id: "copilot",
      eyebrow: "02 / 05 • Autonomous Agent",
    ...
    {
      id: "momentum",
      eyebrow: "03 / 05 • Momentum & Urgency",
      title: "Dynamic Momentum Engine",
      description: "Momentum calculates a real-time health score of your daily productivity based on task urgency, efforts, and streaks. Keep your momentum score high to maintain your ultimate productive state.",
      bullets: [
    ...
    {
      id: "energy",
      eyebrow: "04 / 05 • Mindful Energy Sync",
    ...
    {
      id: "themes",
      eyebrow: "05 / 05 • Adaptive Visual Design",
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 700, letterSpacing: "-0.01em", color: "#fff" }}>MOMENTUM <span style={{ color: "var(--violet)", fontSize: 11, verticalAlign: "super" }}>PRO</span></span>
        </div>
        <button 
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ 
            ...
            color: step === 0 ? "var(--gold)" : "var(--violet)",
            textShadow: step === 0 ? "0 0 8px rgba(255,180,84,0.3)" : "none"
          }}>
            {steps[step].eyebrow}
          </div>
...
export default function App() {
  ...
  const isMobile = deviceMode === "mobile" || (deviceMode === "desktop" && windowWidth < 768);
  const isTablet = deviceMode === "tablet" || (deviceMode === "desktop" && windowWidth >= 768 && windowWidth < 1100);
  ...
  async function connectCalendar() {
    ...
    if (!ok) {
      flash("Calendar connection cancelled.", "warn");
      return;
    }
    ...
    flash(`Google Calendar synced — pulled ${busy.length} busy block${busy.length !== 1 ? "s" : ""}.`, "good");
  }
  ...
  async function weaveStory(goalText: string, specificGenre?: string) {
    ...
    try {
      ...
      flash(`The narrator has woven your ${targetGenre} quest.`, "good");
    } catch (e) {
      patch(d => {
  ...
  function startFocus(t: Task) {
    ...
    setFocusTask(t);
    if (!t.firstStep) {
      const fs = firstStep(t);
      patch(d => { const x = d.tasks.find(z => z.id === t.id); if (x) x.firstStep = fs; });
      setFocusTask({ ...t, firstStep: fs });
      AI.call("first_step", { title: t.title, why: t.why }).then(r => {
        if (r && r.body) {
          patch(d => { const x = d.tasks.find(z => z.id === t.id); if (x) x.firstStep = r; });
          setFocusTask(f => f && f.id === t.id ? { ...f, firstStep: r } : f);
        }
      });
    }
  }
  ...
  function addTask(t: Omit<Task, "id" | "done">) {
    patch(d => d.tasks.unshift({ ...t, id: "t" + now(), done: false }));
  ...
  const tabs = [
    ["mission", "Mission"], ["copilot", "Co-pilot"], ["tasks", "Tasks"], ["habits", "Habits & XP"],
  ...
  return (
    <>
      ...
      <div className="wrap" style={{ padding: isMobile ? "10px" : "24px", maxWidth: deviceMode === "mobile" ? "420px" : deviceMode === "tablet" ? "880px" : "1200px", margin: "0 auto", transition: "all 0.3s ease" }}>
      {/* Interactive Device Selector Panel */}
      <div style={{
        display: "flex",
        flexDirection: isMobile ? "column" : "row",
        justifyContent: "space-between",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ color: "var(--violet)", display: "flex", alignItems: "center" }}><MIcon name="devices" s={16} /></span>
          <span style={{ fontSize: 13, fontWeight: 500, color: "var(--text)" }}>
            Layout Mode: <strong style={{ color: "var(--flow)" }}>{deviceMode.toUpperCase()}</strong>
          </span>
          {deviceMode === "desktop" && <span style={{ fontSize: 11, color: "var(--muted)" }}>(Responsive: {windowWidth}px)</span>}
        </div>
        <div style={{ display: "flex", gap: 6, width: isMobile ? "100%" : "auto", justifyContent: isMobile ? "center" : "flex-end" }}>
          {(["desktop", "tablet", "mobile"] as const).map((m) => (
            <button
              ...
              style={{
                padding: "6px 12px",
                borderRadius: 8,
                border: "1px solid " + (deviceMode === m ? "var(--flow)" : "var(--line)"),
                background: deviceMode === m ? "linear-gradient(180deg,var(--flow),color-mix(in srgb, var(--flow) 80%, #000))" : "var(--surface)",
                color: deviceMode === m ? "var(--on-accent)" : "var(--text)",
                fontWeight: 600,
                fontSize: 12,
                textTransform: "capitalize"
              }}
      ...
      {/* Realistic device simulation wrapper */}
      <div style={{
        border: (deviceMode === "mobile" || deviceMode === "tablet") ? "8px solid rgba(255, 255, 255, 0.08)" : "none",
        borderRadius: deviceMode === "mobile" ? "32px" : deviceMode === "tablet" ? "20px" : "0px",
        padding: (deviceMode === "mobile" || deviceMode === "tablet") ? (isMobile ? "12px 10px" : "20px 16px") : "0px",
        background: (deviceMode === "mobile" || deviceMode === "tablet") ? "rgba(10, 10, 15, 0.4)" : "transparent",
        boxShadow: (deviceMode === "mobile" || deviceMode === "tablet") ? "0 20px 50px rgba(0,0,0,0.5)" : "none",
        minHeight: deviceMode === "mobile" ? "780px" : deviceMode === "tablet" ? "1000px" : "auto",
        ...
        {tab === "mission" && theme === "story" && !db?.settings?.storyModeDisabled && (
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }} className="rise">
            ...
            <Mission db={db} activeTasks={activeTasks} topUrgent={topUrgent} momentum={momentum} onFocus={startFocus} onRescue={setRescueTask} onComplete={completeTask} onMiss={missTask} onLogEnergy={setEnergyFor} patch={patch} isMobile={isMobile} isTablet={isTablet} />
          </div>
        )}
        {tab === "mission" && (theme !== "story" || db?.settings?.storyModeDisabled) && (
          <Mission db={db} activeTasks={activeTasks} topUrgent={topUrgent} momentum={momentum} onFocus={startFocus} onRescue={setRescueTask} onComplete={completeTask} onMiss={missTask} onLogEnergy={setEnergyFor} patch={patch} isMobile={isMobile} isTablet={isTablet} />
        ...
        {tab === "tasks" && <TasksView tasks={activeTasks} onFocus={startFocus} onRescue={setRescueTask} onComplete={completeTask} onMiss={missTask} onLogEnergy={setEnergyFor} isMobile={isMobile} />}
        ...
        <footer style={{ marginTop: 60, paddingTop: 20, borderTop: "1px solid var(--line-soft)", display: "flex", justifyValue: "space-between", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
          <span className="mono" style={{ fontSize: 11, color: "var(--muted-2)", textAlign: isMobile ? "center" : "left", width: isMobile ? "100%" : "auto" }}>MOMENTUM · mission control for not missing your life</span>
          <span className="mono" style={{ fontSize: 11, color: "var(--muted-2)", textAlign: isMobile ? "center" : "right", width: isMobile ? "100%" : "auto" }}><SystemStatusBadge db={db} gcalConnected={gcal.connected} /></span>
        </footer>
      </div>
    </div>
    </>
    ...
    <header className="card hero" style={{ 
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: isMobile ? "center" : "flex-start", gap: 10, flexWrap: "wrap" }}>
          ...
          {auth && auth.enabled && (
            <button onClick={() => auth.user ? Auth.signOut() : Auth.signIn()} className="tap focusable pill"
              style={{ background: "color-mix(in srgb, var(--flow) 12%, transparent)", color: "var(--flow)", border: "1px solid color-mix(in srgb, var(--flow) 30%, transparent)", cursor: "pointer" }}>
              {auth.user ? (<><MIcon name="account_circle" s={13} fill={1} /> {auth.user.displayName?.split(" ")[0] || "Signed in"} · sign out</>) : (<><MIcon name="login" s={13} /> Sign in</>)}
            </button>
          )}
          {gcal && gcal.enabled && (
            <button onClick={() => gcal.connected ? null : onConnectCal()} className="tap focusable pill"
              ...
              <MIcon name="calendar_month" s={13} /> {gcal.connected ? "Google Calendar synced" : "Connect Google Calendar"}
            </button>
          )}
        </div>
...
function Mission({ db, activeTasks, topUrgent, momentum, onFocus, onRescue, onComplete, onMiss, onLogEnergy, patch, isMobile, isTablet }: any) {
        ...
        <div className="card" style={{ padding: "4px 4px 8px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {activeTasks.map((t: any) => (
              <TaskRow key={t.id} t={t} onFocus={onFocus} onRescue={onRescue} onComplete={onComplete} onMiss={onMiss} isMobile={isMobile} />
            ))}
...
function TaskRow({ t, onFocus, onRescue, onComplete, onMiss, isMobile }: any) {
...
function TasksView({ tasks, onFocus, onRescue, onComplete, onMiss, onLogEnergy, isMobile }: any) {
  return (
    <div className="card rise" style={{ padding: isMobile ? "4px 0px 8px" : "4px 4px 8px" }}>
      <div style={{ padding: "16px 18px 8px" }} className="eyebrow">All open tasks · sorted by priority</div>
      {tasks.map((t: any) => <TaskRow key={t.id} t={t} onFocus={onFocus} onRescue={onRescue} onComplete={onComplete} onMiss={onMiss} isMobile={isMobile} />)}
...
function ManualForm({ onAdd, goals }: any) {
  const [f, setF] = useState({ title: "", type: "one-off" as const, dueH: 24, effortMin: 30, stakes: 2, why: "", when: "", goal: goals[0] || "" });
  const set = (k: string, v: any) => setF(s => ({ ...s, [k]: v }));
  const submit = () => {
    if (!f.title.trim()) return;
    onAdd({ title: f.title.trim(), type: f.type, due: now() + f.dueH * HOUR, effortMin: +f.effortMin, stakes: +f.stakes, why: f.why.trim(), when: f.when.trim(), goal: f.type === "habit" ? f.goal : undefined });
    setF({ title: "", type: "one-off", dueH: 24, effortMin: 30, stakes: 2, why: "", when: "", goal: goals[0] || "" });
  };
  return (
    <div className="card" style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <Field label={`Due in ~${f.dueH}h`}>
          <input type="range" min="1" max="168" value={f.dueH} onChange={e => set("dueH", e.target.value)} style={{ width: "100%" }} />
        </Field>
        <Field label={`Effort ~${f.effortMin} min`}>
          <input type="range" min="5" max="240" step="5" value={f.effortMin} onChange={e => set("effortMin", e.target.value)} style={{ width: "100%" }} />
        </Field>
      </div>
      {f.type === "habit" && (
    ...
    <div className="card" style={{ padding: 0, overflow: "hidden", marginBottom: 18 }}>
      ...
      <div style={{ padding: "20px 22px" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 20 }}>
          ...
          <button onClick={() => onWeave(prompt, genre)} disabled={weaving} className="tap focusable"
