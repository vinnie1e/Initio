...
export default function App() {
  ...
  const tabs = [
    ["mission", "MISSION"], ["copilot", "TASKS"], ["tasks", "TRACKER"], ["focus", "FOCUS"], ["habits", "HABITS"]
  ] as const;
  ...
  return (
    <>
      ...
      <div className="wrap" style={{ padding: isMobile ? "10px" : "24px 20px 90px", maxWidth: "1200px", margin: "0 auto", transition: "all 0.3s ease" }}>
            ...
            return (
              <div style={{ margin: isMobile ? "16px 0" : "26px 0 28px" }}>
                ...
                <div className="orbnav" style={{ justifyContent: isMobile ? "center" : "flex-start" }}>
                  {tabs.map(([k, label], i) => (
                    <button key={k} onClick={() => setTab(k)} className={"orb tap focusable" + (tab === k ? " active" : "")} style={{ minWidth: isMobile ? "80px" : undefined, fontFamily: "var(--font-display)", textTransform: "uppercase" }}>
            ...
            return (
              <div style={{ margin: isMobile ? "16px 0" : "24px 0 26px" }}>
                ...
                <div className="rulebar" style={{ justifyContent: isMobile ? "center" : "flex-start" }}>
                  {tabs.map(([k, label]) => (
                    <button key={k} onClick={() => setTab(k)} className={tab === k ? "active" : ""} style={{ fontFamily: "var(--font-display)", textTransform: "uppercase" }}>{label}</button>
                  ))}
                </div>
              </div>
            );
            ...
            <nav style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: isMobile ? "16px 0" : "22px 0 20px", alignItems: "center", justifyContent: isMobile ? "center" : "flex-start" }}>
              {tabs.map(([k, label]) => (
                <button key={k} onClick={() => setTab(k)} className="tap focusable"
                   style={{ padding: isMobile ? "6px 10px" : "9px 15px", borderRadius: "var(--radius-btn)", fontWeight: 600, fontSize: isMobile ? 12 : 13.5, border: "1px solid " + (tab === k ? "var(--ember)" : "var(--line)"), background: tab === k ? "linear-gradient(180deg,color-mix(in srgb, var(--ember) 18%, transparent),color-mix(in srgb, var(--ember) 6%, transparent))" : "transparent", color: tab === k ? "var(--text)" : "var(--muted)", fontFamily: "var(--font-display)", textTransform: "uppercase" }}>
                  {label}
                </button>
              ))}
        ...
        {tab === "copilot" && <AgentView db={db} onApply={applyAgentActions} isMobile={isMobile} autoRunQuery={autoRunQuery} onClearAutoRun={() => setAutoRunQuery("")} onAdd={addTask} goals={Object.keys(db.goals)} />}
        {tab === "tasks" && (
        ...
        {tab === "habits" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 32 }}>
            <HabitsView db={db} patch={patch} flash={flash} />
            <EnergyView db={db} />
          </div>
        )}
  ...
  return (
    <header className="card hero" style={{ 
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "row" : "column", gap: 10, alignItems: isMobile ? "center" : "flex-end", justifyContent: isMobile ? "center" : "flex-start", flexWrap: "wrap" }}>
        ...
        <div title="Current Level and XP Progress"
          style={{ 
          ...
          <span style={{ display: "inline-flex", alignItems: "center", opacity: 0.8 }}>
            <MIcon name="military_tech" s={12} fill={1} />
          </span>
          <span className="mono" style={{ fontWeight: 700, fontSize: 11, lineHeight: 1, letterSpacing: 0.5 }}>
            LVL {level}
          </span>
          <div style={{ width: 1, height: 10, background: "color-mix(in srgb, var(--violet) 20%, transparent)", margin: "0 2px" }} />
          <span className="mono" style={{ fontSize: 9, fontWeight: 500, lineHeight: 1, color: "var(--text)", opacity: 0.7, letterSpacing: 0.5 }}>
            {xpInLevel(xp)} / {LEVEL_STEP} XP
          </span>
        </div>
      </div>
    </header>
...
function AgentView({ db, onApply, isMobile, autoRunQuery, onClearAutoRun, onAdd, goals }: any) {
  const [goal, setGoal] = useState("Apply to midterm paper due Friday.");
  const [phase, setPhase] = useState("idle");
  const [steps, setSteps] = useState<any[]>([]);
  const [result, setResult] = useState<any>(null);
  const [src, setSrc] = useState("local");
  const [viewMode, setViewMode] = useState<"copilot" | "manual" | "extract">("copilot");
  ...
  return (
    <div className="rise" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
        {[["copilot", "◆ AI Co-pilot"], ["manual", "Manual Task"], ["extract", "Extract from Text"]].map(([k, l]) => (
          <button key={k} onClick={() => setViewMode(k as any)} className="tap focusable"
            style={{ padding: "8px 14px", borderRadius: 10, fontWeight: 600, fontSize: 13, border: "1px solid " + (viewMode === k ? (k === "copilot" ? "var(--violet)" : "var(--ember)") : "var(--line)"), background: viewMode === k ? (k === "copilot" ? "color-mix(in srgb, var(--violet) 12%, transparent)" : "color-mix(in srgb, var(--ember) 12%, transparent)") : "transparent", color: viewMode === k ? "#fff" : "var(--muted)" }}>{l}</button>
        ))}
      </div>

      {viewMode === "manual" && <ManualForm onAdd={onAdd} goals={goals} />}
      {viewMode === "extract" && <ExtractForm onAdd={onAdd} />}
      {viewMode === "copilot" && (
